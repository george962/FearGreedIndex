#!/usr/bin/env python3
"""V3-013 controlled ablation for the QQQ/SPY relative-strength feature family."""
...
from v3.features.build_relative_strength_features import (
    ...
    DEFAULT_REPORT as RELATIVE_FEATURE_REPORT,
    DEFAULT_SOURCE as RELATIVE_SNAPSHOT,
    run_build as build_relative_features,
)
from v3.models.common import DEFAULT_FEATURE_REGISTRY, DEFAULT_MODEL_DATASET

ROOT = Path(__file__).resolve().parents[2]
SOURCE_MANIFEST = ROOT / "v3" / "data" / "qqq_spy_source.json"
ABLATION_VERSION = "v3-relative-strength-ablation-001"

FROZEN_BASE_DATASET = (
    ROOT / "v3" / "data" / "model_dataset_relative_strength_baseline_asof_2026_08_18.parquet"
)
FROZEN_RELATIVE_DATASET = (
    ROOT / "v3" / "data" / "model_dataset_relative_strength_asof_2026_08_18.parquet"
)

BASE_PREDICTIONS = ROOT / "v3" / "reports" / "relative_strength_ablation_baseline_predictions.parquet"
BASE_METRICS = ROOT / "v3" / "reports" / "relative_strength_ablation_baseline_metrics.csv"
BASE_SUMMARY = ROOT / "v3" / "reports" / "relative_strength_ablation_baseline_summary.json"
RELATIVE_PREDICTIONS = ROOT / "v3" / "reports" / "relative_strength_ablation_candidate_predictions.parquet"
RELATIVE_METRICS = ROOT / "v3" / "reports" / "relative_strength_ablation_candidate_metrics.csv"
RELATIVE_SUMMARY = ROOT / "v3" / "reports" / "relative_strength_ablation_candidate_summary.json"
COMPARISON_OUTPUT = ROOT / "v3" / "reports" / "relative_strength_ablation_comparison.csv"
LANE_OUTPUT = ROOT / "v3" / "reports" / "relative_strength_ablation_lane_summary.csv"
TOURNAMENT_OUTPUT = ROOT / "v3" / "reports" / "relative_strength_ablation_tournament.csv"
REPORT_OUTPUT = ROOT / "v3" / "reports" / "relative_strength_ablation.json"
...
def verify_snapshot() -> dict[str, Any]:
    if not RELATIVE_SNAPSHOT.exists() or not SOURCE_MANIFEST.exists():
        raise ValueError("V3-013 requires the checked-in QQQ/SPY snapshot and manifest")
    manifest = json.loads(SOURCE_MANIFEST.read_text(encoding="utf-8"))
    expected = str(manifest.get("snapshot_sha256", ""))
    actual = _sha256(RELATIVE_SNAPSHOT)
    if not expected or expected != actual:
        raise ValueError(
            "QQQ/SPY snapshot hash does not match its manifest; do not silently refetch"
        )
    return manifest
...
def prepare_frozen_datasets() -> dict[str, Any]:
    ...
    frozen_candidate.to_parquet(FROZEN_RELATIVE_DATASET, index=False, engine="pyarrow")
    ...
    return {
        ...
        "candidate_dataset_sha256": _sha256(FROZEN_RELATIVE_DATASET),
    }


def relative_strength_family_decision(lanes: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "retain_relative_strength": retain,
        "decision": (
            "KEEP_RELATIVE_STRENGTH_FOR_LATER_RESEARCH"
            if retain
            else "DO_NOT_RETAIN_RELATIVE_STRENGTH_FEATURE_FAMILY"
        ),
        ...
        "criterion": (
            ...
            "at least 2 of 3 chronological folds improve. Relative strength is "
            "retained only when at least 2 of 3 lanes are robust in both EXP-003 "
            "and EXP-004."
        ),
    }
...
def build_tournament(
    ...
    candidate["experiment_id"] = "RS-" + candidate["experiment_id"].astype(str)
    candidate["model_name"] = candidate["model_name"].astype(str) + " [+QQQ/SPY]"
    return summarize_experiments(
        pd.concat([baseline, candidate], ignore_index=True, sort=False)
    )


def run_relative_strength_ablation(
    ...
    candidate_report = run_common_evaluation(
        dataset_path=FROZEN_RELATIVE_DATASET,
        registry_path=RELATIVE_REGISTRY,
        predictions_output=RELATIVE_PREDICTIONS,
        metrics_output=RELATIVE_METRICS,
        summary_output=RELATIVE_SUMMARY,
    )

    baseline_metrics = pd.read_csv(BASE_METRICS)
    candidate_metrics = pd.read_csv(RELATIVE_METRICS)
    ...
    report: dict[str, Any] = {
        "status": "RELATIVE_STRENGTH_ABLATION_COMPLETE",
        ...
        "next": "V3-014 market-breadth features",
        "note": (
            "V3-013 changes only the QQQ/SPY feature family. The experiment is frozen "
            "as-of 2026-08-18 and does not promote a model or tune a trading policy."
        ),
    }
...
def main() -> int:
    args = parse_args()
    report = run_relative_strength_ablation(args.report_output)
    return 0 if report["status"] == "RELATIVE_STRENGTH_ABLATION_COMPLETE" else 1
