...
The engine consumes immutable evaluation evidence and decides only whether a
candidate is eligible for V3-019. It never trains a model, changes policy,
changes sizing, or selects a champion.
"""
...
def _leq(value: float, limit: float) -> bool:
    """Inclusive registered boundary with floating-point comparison tolerance."""
    return value <= limit + NUMERIC_EPSILON


def _geq(value: float, limit: float) -> bool:
    """Inclusive registered boundary with floating-point comparison tolerance."""
    return value + NUMERIC_EPSILON >= limit
...
def _gate(
    ...
    *,
    ...
    return {"status": status, "reason": reason, "details": details or {}}
...
def _blocked_after_prediction(name: str) -> dict[str, Any]:
    return _gate(
        "BLOCKED",
        reason=(
            f"{name} cannot be used for promotion because the prediction "
            "prerequisite failed; V3-017 extra sizing remains locked."
        ),
    )
...
def _evaluate_calibration(
    ...
    values = evidence.get("calibration")
    limits = config["calibration"]
    if not isinstance(values, dict):
        return _gate("FAIL", reason="Calibration evidence is missing.")
    raw = (
        values.get("mean_ece"),
        values.get("max_fold_horizon_ece"),
        values.get("mean_relative_brier_improvement"),
    )
    if not _finite_numbers(raw):
        return _gate(
            "FAIL",
            reason="Calibration evidence contains missing or non-finite values.",
        )
    mean_ece, max_ece, brier = map(float, raw)
    passed = (
        _leq(mean_ece, float(limits["mean_ece_max"]))
        and _leq(max_ece, float(limits["max_fold_horizon_ece"]))
        and brier
        > float(limits["mean_relative_brier_improvement_min_exclusive"])
    )
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            ...
            if passed
            else "Calibration thresholds fail."
        ),
        details={
            "mean_ece": mean_ece,
            "max_fold_horizon_ece": max_ece,
            "mean_relative_brier_improvement": brier,
        },
    )


def _evaluate_after_cost_portfolio(
    evidence: dict[str, Any],
    config: dict[str, Any],
) -> dict[str, Any]:
    values = evidence.get("portfolio")
    limits = config["after_cost_portfolio"]
    if not isinstance(values, dict):
        return _gate("FAIL", reason="After-cost portfolio evidence is missing.")
    raw = (
        values.get("base_cost_bps_per_1x_turnover"),
        values.get("combined_excess_total_return"),
        values.get("annualized_excess_return"),
    )
    if not _finite_numbers(raw):
        return _gate(
            "FAIL",
            reason="After-cost portfolio evidence is missing or non-finite.",
        )
    cost, total_excess, annual_excess = map(float, raw)
    passed = (
        math.isclose(
            cost,
            float(limits["base_cost_bps_per_1x_turnover"]),
            rel_tol=0.0,
            abs_tol=NUMERIC_EPSILON,
        )
        and total_excess
        > float(limits["combined_excess_total_return_min_exclusive"])
        and annual_excess
        > float(limits["annualized_excess_return_min_exclusive"])
    )
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            "Base-cost portfolio edge passes."
            if passed
            else "Base-cost portfolio edge fails."
        ),
        details={
...
def _evaluate_cross_year(
    evidence: dict[str, Any],
    config: dict[str, Any],
) -> dict[str, Any]:
    values = evidence.get("cross_year")
    limits = config["cross_year_robustness"]
    if not isinstance(values, dict):
        return _gate("FAIL", reason="Cross-year evidence is missing.")
    fold_excess = values.get("fold_excess_total_returns")
    if (
        not isinstance(fold_excess, list)
        or len(fold_excess) != int(limits["expected_folds"])
    ):
        return _gate(
            "FAIL",
            reason="Cross-year evidence has the wrong number of folds.",
        )
    if not _finite_numbers(fold_excess):
        return _gate(
            "FAIL",
            reason="Cross-year evidence contains missing/non-finite values.",
        )
    folds = [float(value) for value in fold_excess]
    positive = [value for value in folds if value > 0.0]
    positive_count = len(positive)
    if positive:
        total_positive = sum(positive)
        max_share = (
            max(positive) / total_positive
    ...
    passed = (
        ...
        and _leq(
            max_share,
            float(limits["max_positive_excess_contribution_share"]),
        )
    )
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            "Cross-year robustness passes."
            if passed
            else "Cross-year robustness fails."
        ),
        details={
            "fold_excess_total_returns": folds,
...
def _evaluate_sharpe(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    if not _finite_numbers(raw):
        return _gate(
            "FAIL",
            reason="Sharpe evidence contains missing/non-finite values.",
        )
    candidate, benchmark, v2_1 = map(float, raw)
    passed = _geq(candidate, benchmark) and _geq(candidate, v2_1)
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            "Sharpe is at least both references."
            if passed
            else "Sharpe is below a required reference."
        ),
        details={
...
def _drawdown_ceiling(
    ...
    hard_pp = float(
        config["parameter_robustness"][
            "hard_drawdown_ceiling_deterioration_pp"
        ]
    )
...
def _evaluate_drawdown(
    ...
    if not isinstance(risk, dict) or not isinstance(portfolio, dict):
        return _gate(
            "FAIL",
            reason="Drawdown or return-benefit evidence is missing.",
        )
    raw = (
    ...
    if not _finite_numbers(raw):
        return _gate(
            "FAIL",
            reason="Drawdown evidence contains missing/non-finite values.",
        )
    (
        candidate_dd,
        benchmark_dd,
    ...
    if min(candidate_dd, benchmark_dd, v2_1_dd) < 0.0:
        return _gate(
            "FAIL",
            reason="Maximum drawdown must be supplied as non-negative magnitudes.",
        )
    ...
    min_benefit = float(
        limits["conditional_min_annualized_return_benefit_pp"]
    )

    if _leq(deterioration_pp, simple_limit):
        passed = True
        route = "within_no_exception_limit"
    elif _leq(deterioration_pp, conditional_limit):
        passed = (
            _geq(annual_excess * 100.0, min_benefit)
            and _geq(candidate_calmar, benchmark_calmar)
            and _geq(candidate_calmar, v2_1_calmar)
        )
    ...
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            "Maximum-drawdown gate passes."
            if passed
            else "Maximum-drawdown gate fails."
        ),
        details={
...
def _evaluate_cost_robustness(
    evidence: dict[str, Any],
    config: dict[str, Any],
) -> dict[str, Any]:
    values = evidence.get("cost_robustness")
    limits = config["cost_robustness"]
    if not isinstance(values, dict):
        return _gate("FAIL", reason="Cost-robustness evidence is missing.")
    raw = (
        values.get("stress_5bps_excess_total_return"),
        values.get("stress_10bps_annualized_excess_return"),
    )
    if not _finite_numbers(raw):
        return _gate(
            "FAIL",
            reason="Cost-robustness evidence is missing or non-finite.",
        )
    excess_5, annual_10 = map(float, raw)
    passed = (
        excess_5
        > float(limits["stress_5bps_excess_total_return_min_exclusive"])
        and _geq(
            annual_10,
            float(
                limits[
                    "stress_10bps_annualized_excess_return_min_inclusive"
                ]
            ),
        )
    )
...
def _evaluate_parameter_robustness(
    ...
    if not isinstance(parameter, dict):
        return _gate(
            "FAIL",
            reason="Parameter-robustness evidence is missing.",
        )
    ...
    if not isinstance(scenarios, list) or len(scenarios) != expected:
        return _gate(
            "FAIL",
            reason="Parameter robustness has the wrong scenario count.",
        )
    ceiling = _drawdown_ceiling(evidence, config)
    if ceiling is None:
        return _gate(
            "FAIL",
            reason="Cannot compute parameter-robustness drawdown ceiling.",
        )
    ...
    for index, scenario in enumerate(scenarios):
        ...
        if not _finite_numbers((excess, drawdown)):
            return _gate(
                "FAIL",
                reason="Parameter scenario contains missing/non-finite values.",
            )
        ...
        if drawdown_value < 0.0:
            return _gate(
                "FAIL",
                reason="Parameter-scenario drawdown must be non-negative.",
            )
        if excess_value > float(
            limits["positive_excess_total_return_min_exclusive"]
        ):
            positive_count += 1
        if not _leq(drawdown_value, ceiling):
    ...
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            "Parameter perturbations pass."
            if passed
            else "Parameter perturbations fail."
        ),
        details={
...
def _evaluate_data_quality(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    return _gate(
        "PASS" if passed else "FAIL",
        reason=(
            "Data-quality and leakage gates pass."
            if passed
            else "A data-quality/leakage gate failed or is missing."
        ),
...
def evaluate_candidate(
    ...
    gates: dict[str, dict[str, Any]] = {
        "evidence_completeness": _evaluate_evidence_completeness(evidence),
        "prediction_prerequisite": _evaluate_prediction(evidence),
    }
    ...
    if not prediction_pass:
        ...
        gates["after_cost_portfolio"] = _evaluate_after_cost_portfolio(
            evidence, config
        )
        gates["cross_year_robustness"] = _evaluate_cross_year(
            evidence, config
        )
        gates["sharpe"] = _evaluate_sharpe(evidence)
        gates["maximum_drawdown"] = _evaluate_drawdown(evidence, config)
        gates["cost_robustness"] = _evaluate_cost_robustness(
            evidence, config
        )
        gates["parameter_robustness"] = _evaluate_parameter_robustness(
            evidence, config
        )
    ...
    promotion_ready = all(
        gate["status"] == "PASS" for gate in ordered.values()
    )
    failures = [
        name for name, gate in ordered.items() if gate["status"] != "PASS"
    ]
    return {
        ...
        "status": (
            "PROMOTION_READY" if promotion_ready else "NOT_PROMOTION_READY"
        ),
        ...
        "gates": ordered,
        "note": (
            "V3-018 determines promotion eligibility only. V3-019 must still "
            "select exactly one passing candidate before any champion manifest "
            "or extra sizing can activate."
        ),
    }
