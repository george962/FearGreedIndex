...
def main() -> int:
    ...
    require(
        evidence.get("v3/reports/exp009_evaluation.json") == sha256_bytes(frozen_evaluation_bytes),
        "EXP-009 committed evaluation hash mismatch",
    )
    require(
        evidence.get("v3/reports/exp009_metrics.csv") == sha256_bytes(frozen_metrics_bytes),
        "EXP-009 committed metrics hash mismatch",
    )
