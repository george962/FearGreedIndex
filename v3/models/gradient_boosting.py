#!/usr/bin/env python3
"""Fixed histogram-gradient-boosting candidate for v3.

The candidate predicts positive-return probabilities and expected returns for
5/20/60 sessions plus expected 20-session maximum drawdown. Hyperparameters are
fixed before evaluating the chronological folds; this task does not tune them or
select a champion.
"""
...
DEFAULT_PREDICTIONS = ROOT / "v3" / "reports" / "gradient_boosting_predictions.parquet"
DEFAULT_SUMMARY = ROOT / "v3" / "reports" / "gradient_boosting_summary.json"
...
def fit_probability(
    ...
    result = pipeline.predict_proba(x_test)[:, positive_index]
...
def fit_regression(
    ...
    pipeline.fit(x_train, y)
    result = np.asarray(pipeline.predict(x_test), dtype=float)
    if not np.isfinite(result).all():
        raise ValueError("Gradient boosting produced non-finite predictions")
    return result
...
def summarize_fold(
    ...
    for horizon in HORIZONS:
        class_pred = f"predicted_p_up_{horizon}d"
        class_target = f"forward_positive_{horizon}d"
        class_joined = predictions[["decision_date", class_pred]].merge(
        ...
        if len(realized_class):
            ...
            p = pd.to_numeric(realized_class[class_pred], errors="raise").to_numpy(float)
            rows.append(
                {
                    "fold": str(fold["name"]),
        ...
        return_pred = f"predicted_return_{horizon}d"
        return_target = f"forward_return_{horizon}d"
        return_joined = predictions[["decision_date", return_pred]].merge(
        ...
        if len(realized_return):
            ...
            pred = pd.to_numeric(realized_return[return_pred], errors="raise").to_numpy(float)
            error = pred - actual
            rows.append(
                {
                    "fold": str(fold["name"]),
...
def run_candidate(
    ...
    report = {
        ...
        "note": "V3-007 fixed nonlinear candidate; no champion selection or post-hoc tuning.",
    }
