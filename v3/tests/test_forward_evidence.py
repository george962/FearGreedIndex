...
class ForwardEvidenceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.frozen_treasury = self.root / "treasury_frozen.csv"
        self.forward_treasury = self.root / "treasury_forward.csv"
        ...
        self.frozen_treasury.write_text(
            "date,dgs2,dgs10\n2026-08-17,3.5,4.0\n2026-08-18,3.6,4.1\n",
            encoding="utf-8",
        )
        self.forward_treasury.write_text(
            "observation_date,dgs2,dgs10,captured_on_date,dgs2_source_sha256,dgs10_source_sha256,previous_row_sha256,row_sha256\n",
            encoding="utf-8",
        )
    ...
    def _verify(self) -> dict[str, object]:
        return verify_lane(
            ...
            treasury_forward_source_path=self.forward_treasury,
            treasury_frozen_source_path=self.frozen_treasury,
        )
    ...
    def test_append_is_idempotent_and_chain_verifies(self) -> None:
        ...
        self.assertEqual(report["treasury_forward_source"]["rows"], 0)
    ...
    def test_forward_treasury_capture_is_append_only_and_as_of_materialized(self) -> None:
        def fake_fetcher(**_: object) -> tuple[pd.DataFrame, dict[str, str]]:
            return (
                pd.DataFrame(
                    {
                ...
                {"DGS2": "a" * 64, "DGS10": "b" * 64},
            )

        frozen_before = self.frozen_treasury.read_bytes()
        appended = collect_forward_treasury(
            capture_date="2026-08-20",
            frozen_source_path=self.frozen_treasury,
            forward_source_path=self.forward_treasury,
            manifest_path=self.manifest,
            fetcher=fake_fetcher,
        )
        self.assertEqual(appended, 2)
        self.assertEqual(self.frozen_treasury.read_bytes(), frozen_before)
        report = verify_forward_treasury_source(
            self.forward_treasury,
            frozen_source_path=self.frozen_treasury,
        )
        self.assertEqual(report["rows"], 2)

        source_819 = self.root / "source_819.csv"
        materialize_source_for_decision(
            decision_date="2026-08-19",
            output_path=source_819,
            frozen_source_path=self.frozen_treasury,
            forward_source_path=self.forward_treasury,
        )
        frame_819 = pd.read_csv(source_819)
        # Both rows were first captured on 8/20, so neither may appear in an 8/19 snapshot.
        self.assertEqual(frame_819["date"].iloc[-1], "2026-08-18")

        source_820 = self.root / "source_820.csv"
        materialize_source_for_decision(
            decision_date="2026-08-20",
            output_path=source_820,
            frozen_source_path=self.frozen_treasury,
            forward_source_path=self.forward_treasury,
        )
        frame_820 = pd.read_csv(source_820)
        self.assertEqual(frame_820["date"].iloc[-1], "2026-08-20")

    def test_forward_treasury_tampering_is_detected(self) -> None:
        def fake_fetcher(**_: object) -> tuple[pd.DataFrame, dict[str, str]]:
            return (
                pd.DataFrame(
                    {"date": pd.to_datetime(["2026-08-19"]), "dgs2": [3.7], "dgs10": [4.2]}
                ),
                {"DGS2": "a" * 64, "DGS10": "b" * 64},
            )
        ...
        text = self.forward_treasury.read_text(encoding="utf-8").replace("3.7000000000000002", "9.9")
        if text == self.forward_treasury.read_text(encoding="utf-8"):
            text = text.replace(",3.7,", ",9.9,")
        self.forward_treasury.write_text(text, encoding="utf-8")
        with self.assertRaises(ValueError):
            verify_forward_treasury_source(
                self.forward_treasury,
                frozen_source_path=self.frozen_treasury,
